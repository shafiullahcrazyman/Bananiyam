...
interface Props {
  ...
  onBack?: () => void; // Kept for interface compatibility but not used for nav logic
}
...
const GameMode: React.FC<Props> = ({ settings, setHeaderConfig, updateSettings }) => {
  const { variant } = useParams<{ variant: GameVariant }>();
  ...
  // --- ROUTING SYNC EFFECT ---
  // If URL has variant but we are in MENU state, initialize.
  // If URL has no variant (Arcade Menu) but we are in GAME state, reset to MENU.
  useEffect(() => {
    if (!variant) {
      if (gameState !== GameState.MENU && gameState !== GameState.WHEEL_SPIN) {
        setGameState(GameState.MENU);
      }
      return;
    }

    // We have a variant in URL
    if (gameState === GameState.MENU) {
      // User navigated directly to /game/VARIANT or we just routed here
      const targetVariant = variant as GameVariant;
      setGameVariant(targetVariant);
      ...
      if (targetVariant === 'ADVENTURE') {
         setGameState(GameState.ADVENTURE_MAP);
      } else if (needsDifficulty.includes(targetVariant)) {
         setGameState(GameState.DIFFICULTY_SELECT);
      } else {
         // Fallback for modes that might not need difficulty (e.g. WHEEL results)
         initGame(targetVariant, Difficulty.MEDIUM);
      }
    }
  }, [variant, gameState]);
  ...
  useEffect(() => {
    if (!variant) {
      ...
      setHeaderConfig({ 
          title: GAME_TITLES[variant], 
          color: GAME_COLORS[variant] || 'primary'
      });
    }
    
    // Cleanup on unmount
    return () => setHeaderConfig({ title: null, color: "primary" });
  }, [variant, setHeaderConfig]);
  ...
  const handleGameSelect = (selectedVariant: GameVariant) => {
    GameAudio.playClick();
    // Navigate to the game route. The useEffect will handle state transition.
    navigate(`/game/${selectedVariant}`);
  };
  ...
  const initGame = async (targetVariant: GameVariant, selectedDifficulty: Difficulty) => {
    GameAudio.playClick();
    setGameVariant(targetVariant);
    ...
    const loadData = async (offline: boolean) => {
        const count = (targetVariant === 'SPEED' || targetVariant === 'BOSS') ? 15 : 5;
        
        if (offline) {
             if (targetVariant === 'HOMOPHONE') return await OfflineService.generateHomophones(selectedDifficulty, 5);
             if (targetVariant === 'DAILY') return await OfflineService.generateDailyWord(selectedDifficulty);
             if (targetVariant === 'BOSS') return await OfflineService.generateWordList(Difficulty.EXTREME, 10);
             ...
             if (targetVariant === 'HOMOPHONE') return await generateHomophones(selectedDifficulty, 5);
             if (targetVariant === 'DAILY') return await generateDailyWord(selectedDifficulty);
             if (targetVariant === 'BOSS') return await generateWordList(selectedDifficulty, 10, 'BOSS');
             if (targetVariant === 'SILENT_LETTER') return await generateWordList(selectedDifficulty, 5, 'SILENT_LETTER');
             return await generateWordList(selectedDifficulty, count, targetVariant === 'ADVENTURE' ? journeyTheme : undefined);
        }
    };
    ...
    if (['CLASSIC', 'SPEED', 'WHISPER', 'REVERSE', 'ADVENTURE', 'BOSS', 'DAILY', 'SILENT_LETTER', 'MULTIPLAYER'].includes(targetVariant)) {
        setTimeout(() => {
           if (loadedWords.length > 0) {
             // Cast to any to safely access 'word' property if it exists
             const firstItem = loadedWords[0] as any;
             if (firstItem.word) {
                 playWord(firstItem.word, targetVariant === 'WHISPER');
             }
           }
        }, 500);
    }
  ...
  const spinWheel = () => {
    ...
    setTimeout(() => {
        ...
        GameAudio.playWin();
        // Just navigate to the random game
  ...
  // NOTE: If no variant in URL (and state synced), render Menu
  if (!variant || gameState === GameState.MENU) {
    return (
      <div className="flex flex-col items-center animate-fade-in pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-5xl px-4">
          ...
          <MenuButton 
            ...
            color="lime" 
            onClick={() => {
                // For Adventure, we need to handle special routing or state
                navigate('/game/ADVENTURE');
            }} 
          />
  ...
  if (gameState === GameState.DIFFICULTY_SELECT) {
    return (
      <div className="flex flex-col items-center animate-fade-in pb-10 min-h-[60vh] justify-center">
        ...
        <button onClick={handleQuitGame} className="mt-8 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 font-medium">Cancel</button>
      </div>
    );
  }
          ...
          return (
              <div className="flex flex-col items-center animate-fade-in w-full max-w-2xl mx-auto px-4">
                  <div className="flex w-full justify-start items-center mb-8">
                     <button onClick={handleQuitGame} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white font-bold flex gap-2"><RotateCcw/> Exit</button>
                  </div>
      ...
      return (
          <div className="flex flex-col items-center animate-fade-in w-full max-w-4xl mx-auto">
              <div className="flex w-full justify-between items-center mb-8 px-4">
                  <button onClick={handleQuitGame} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white font-bold flex gap-2"><RotateCcw/> Exit Map</button>
      ...
      <div className="w-full max-w-2xl mx-auto p-6 animate-fade-in-up">
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 text-center mb-8 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 flex-wrap">
            ...
            <button onClick={handleQuitGame} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-primary-600 text-white px-8 py-3 rounded-full font-semibold hover:bg-primary-700 transition-colors shadow-lg shadow-primary-200 dark:shadow-none">
  ...
  return (
    <div className="w-full max-w-xl mx-auto px-4 py-8 animate-fade-in relative">
      <div className="absolute top-4 left-4 md:left-0 z-10">
        <button onClick={handleQuitGame} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-sm font-medium">Quit</button>
      </div>
        ...
        <div className="text-center mb-8">
          ...
          {gameVariant === 'MISSING_LETTER' && (
             <div className="mb-8 w-full overflow-hidden">
                ...
                <div className="text-2xl md:text-4xl font-mono font-bold tracking-widest break-all text-slate-800 dark:text-white uppercase leading-relaxed">{maskedWord}</div>
             </div>
          )}
